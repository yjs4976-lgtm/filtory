...
export function SubscriptionManagement() {
  ...
  const cancelCopy = paymentFeatureEnabled ? cancellationCopy.real : cancellationCopy.test
  ...
  const cancel = async () => {
    ...
    try {
      await performSubscriptionCancellation(async () => {
        if (paymentFeatureEnabled && entitlement.provider === "TOSS") {
  ...
  return <div className={styles.subscriptionStack}>
    <section className={styles.subscriptionPlanCard}>
      ...
      <h1>{isPlus ? (paymentFeatureEnabled ? "Filtory Plus" : "Filtory Plus 테스트") : "Free"}</h1>
      ...
      {isPlus && !cancelScheduled && <p>{paymentFeatureEnabled ? `현재 이용 기간은 ${periodEnd}까지이며 매월 자동 갱신됩니다.` : `서버 테스트용 Plus 이용 기간 ${periodEnd}까지 실제 결제나 자동 갱신은 진행되지 않습니다.`}</p>}
    ...
    {isPlus && !cancelScheduled && step === "idle" && <button type="button" className={styles.subscriptionCancelLink} onClick={() => { setError(""); setStep("reason") }}>{cancelCopy.link}</button>}
